...
module.exports = class RoomController extends Controller
	...
	@stanzas:
		...
		# handleItems: (stanza) -> stanza.is('iq') and stanza.attrs.type is 'result' and stanza.attrs.from?.match(/@conference/) and stanza.getChild('query', 'http://jabber.org/protocol/disco#items')?
	...
	# handleItems: (stanza) ->
	# 	items = stanza.getChild('query').getChildren('item')

	# 	jids = []
	# 	for item in items
	# 		jids.push item.attrs.ojid

	# 	@emit 'roomItems:'+stanza.attrs.id, {participants:jids}
	...
	handleInvite: (stanza) ->
		...
		if stanza.getChild('x','http://jabber.org/protocol/muc#user')?.getChild('invite')?
			@emit 'invite', {from:stanza.attrs.from, room:stanza.attrs.to}
		else 
			@emit 'invite', {from:stanza.attrs.from, room:stanza.getChild('x', 'jabber:x:conference').attrs.jid}
